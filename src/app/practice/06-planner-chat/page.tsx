      ...
      <div key={id} className={`flex flex-col ${level > 0 ? "ml-4 border-l border-indigo-900/30 pl-4" : "mb-6"}`}>
        <div className="flex items-start gap-3 group">
          <div className="mt-1 flex flex-col items-center">
            <span className={`flex h-5 w-5 shrink-0 items-center justify-center rounded-full text-[10px] font-bold shadow-sm transition-colors ${
      ...
      <aside className="w-[350px] border-r border-slate-800/60 bg-slate-900/20 hidden xl:flex flex-col">
        ...
        <div className="p-6 bg-black/40 border-t border-slate-800/60">
          ...
          <div className="h-1.5 w-full bg-slate-800 rounded-full overflow-hidden">
            <div className={`h-full bg-indigo-500 transition-all duration-700 shadow-[0_0_10px_rgba(99,102,241,0.5)] ${
              isLoading ? "w-[60%] animate-[shimmer_2s_infinite]" : "w-full"
            }`} />
          </div>
        </div>
      </aside>
        ...
        <div className="flex-1 overflow-y-auto px-8 py-10 space-y-10 scrollbar-thin scrollbar-thumb-slate-800">
          ...
          {messages.map((msg, idx) => (
            <div key={idx} className={`flex gap-8 ${msg.role === "user" ? "flex-row-reverse" : ""}`}>
              <div className={`h-12 w-12 shrink-0 rounded-2xl flex items-center justify-center text-2xl shadow-2xl transition-transform hover:scale-105 ${
                msg.role === "user" 
              ...
              <div className={`max-w-[85%] ${msg.role === "user" ? "bg-indigo-600/90 text-white px-6 py-4 rounded-[2rem] rounded-tr-none shadow-xl" : "flex-1"}`}>
                ...
                <div className={`mt-4 flex items-center gap-2 text-[9px] font-black uppercase tracking-[0.2em] ${
                  msg.role === "user" ? "text-indigo-200/60" : "text-slate-600"
                }`}>
