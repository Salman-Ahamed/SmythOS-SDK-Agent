...
import { featuredExamples, sdkExamples } from "@/lib/db";
...
const ComplexityStars: FC<{ level: number }> = ({ level }) => (
    <div className="flex items-center gap-0.5">
        {Array.from({ length: 5 }).map((_, i) => (
            <StarIcon
                key={i}
                className={`h-3.5 w-3.5 ${i < level ? "text-amber-400" : "text-gray-700"
            ...
            <div className="container mx-auto px-6">
                ...
                <div className="mx-auto mb-16 max-w-3xl text-center">
                    <div className="mb-4 inline-flex items-center gap-2 rounded-full border border-violet-500/30 bg-violet-500/10 px-4 py-2">
                        ...
                        <span className="text-sm font-medium text-violet-400">Featured Examples</span>
                    </div>
                    ...
                    <p className="text-lg text-gray-400">
                        Master AI agent development from basic chat to multi-agent orchestration.
                        Each example teaches new SDK features step by step.
                    </p>
                </div>
                ...
                <div className="mt-12 text-center">
                    <Link
                        href="/practice"
                        className="group inline-flex items-center gap-3 rounded-full border border-violet-500/30 bg-violet-500/10 px-6 py-3 text-lg font-medium text-violet-400 transition-all duration-300 hover:border-violet-500/50 hover:bg-violet-500/20 hover:text-violet-300"
                    >
                        <span>View All {sdkExamples.length} Examples</span>
